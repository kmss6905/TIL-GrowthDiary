...
참고로, Transaction 을 길게 잡는 다면 그 만큼 스레드 당 DB 커넥션 을 점유하는 시간이 늘어날 수 밖에 없다. 이는 커넥션 풀을 더 빨리 마르게 하는 요인이 될 수 있다. 즉, 빨리 커넥션 을 사용하고 DBCP 에 반납해줘야 하는 데 불필요하게 오래 잡고 있다보니 뒤이어 오는 요청 슬레드 들은 커넥션 을 얻을 때 까지 기다릴 수 밖에 없다. 따라서 응답시간은 길어지게 된다.
